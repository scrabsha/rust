...
impl<'tcx> TypeSuperFoldable<TyCtxt<'tcx>> for Ty<'tcx> {
    fn try_super_fold_with<F: FallibleTypeFolder<TyCtxt<'tcx>>>(
        ...
        let kind = match *self.kind() {
            ...
            ty::View(tid, args, fields) => ty::View(tid, args.try_fold_with(folder)?, fields.try_fold_with(folder)?),
    ...
    fn super_fold_with<F: TypeFolder<TyCtxt<'tcx>>>(self, folder: &mut F) -> Self {
        let kind = match *self.kind() {
            ...
            ty::View(tid, args, fields) => ty::View(tid, args.fold_with(folder), fields.fold_with(folder)),
            ty::Dynamic(trait_ty, region) => {
